-- Alertas operacionais para o webhook de assinatura Mercado Pago
-- Uso: executar individualmente ou via scripts/run-mp-subscription-alert-checks.ps1
-- [summary]
SELECT status,
    COUNT(*) AS total
...
WHERE provider = 'mercadopago_subscription'
    AND (
        status IS NULL
        OR (
            status IN ('processed', 'skipped', 'failed')
...
WITH event_stats AS (
    SELECT COUNT(*) FILTER (
        ...
        COUNT(*) FILTER (
            WHERE ignored_reason = 'out_of_order_state_regression'
        ) AS regressions_blocked_24h,
        COUNT(*) FILTER (
