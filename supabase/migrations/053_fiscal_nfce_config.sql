...
CREATE TABLE IF NOT EXISTS fiscal_config (
  ...
  codigo_municipio TEXT NOT NULL, -- Código IBGE 7 dígitos
  uf TEXT NOT NULL CHECK (char_length(uf) = 2),
  cep TEXT NOT NULL,

  -- Certificado A1 (criptografado)
  certificado_storage_path TEXT, -- caminho no Supabase Storage (bucket privado)
  certificado_senha_encrypted TEXT, -- senha do .pfx (protegida por RLS, só o dono lê)
  ...
  cfop_padrao TEXT NOT NULL DEFAULT '5102',     -- Venda mercadoria
...
CREATE OR REPLACE FUNCTION get_next_nfce_number(p_restaurant_id UUID)
RETURNS INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  SET proximo_numero_nfce = proximo_numero_nfce + 1,
      updated_at = NOW()
  WHERE restaurant_id = p_restaurant_id AND ativo = true
  RETURNING proximo_numero_nfce - 1 INTO v_numero;

  IF v_numero IS NULL THEN
    RAISE EXCEPTION 'Configuração fiscal não encontrada ou inativa para restaurant_id %', p_restaurant_id;
  END IF;
...
CREATE POLICY fiscal_notas_owner ON fiscal_notas_emitidas
  FOR ALL
  USING (
    restaurant_id IN (
      SELECT id FROM restaurants WHERE user_id = auth.uid()
